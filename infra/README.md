...
Phaser 游戏和 Babylon 模型查看器使用以下结构部署：

- 私有 S3 Bucket 根目录保存 `phaser3-game/dist`。
- S3 的 `/babylon/` 路径保存 `babylon-viewer/dist-aws`。
...
脚本会创建或更新 CloudFormation 栈，测试并构建两个子项目，上传 S3，然后刷新 CloudFront 缓存。以后每次需要发布新版本时，重新执行同一条命令即可。
